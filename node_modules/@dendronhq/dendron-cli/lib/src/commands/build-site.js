...
function getRoot(engine) {
    ...
    return common_server_1.vault2Path({ vault, wsRoot });
}
...
function makeCygdrivePath(pathname) {
    ...
    pathname = pathname.replace(new RegExp(lodash_1.default.escapeRegExp(path_1.default.sep), "g"), "/");
...
async function note2JekyllMdFile(note, opts) {
    ...
    if (lodash_1.default.some(opts.privateVaults || [], (ent) => {
        return common_all_1.VaultUtils.isEqual(note.vault, { fsPath: ent }, opts.engine.wsRoot);
    })) {
        return [];
    }
