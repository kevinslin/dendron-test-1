...
describe("goUpTo", () => {
    ...
    beforeEach(() => {
        ...
        cwd = path_1.default.join(root, "foo", "bar");
        fs_extra_1.default.ensureDirSync(cwd);
    });
    test("basic", () => {
        expect(filesv2_1.goUpTo(cwd, "foo")).toEqual(path_1.default.join(root));
    });
    // used in dendron-cli/src/commands/build-site-v2.ts
    test("double", () => {
        let cwd2 = path_1.default.join(cwd, "foo", "bar");
        fs_extra_1.default.ensureDirSync(cwd2);
        const match1 = filesv2_1.goUpTo(cwd2, "foo");
        expect(match1).toEqual(cwd);
        expect(filesv2_1.goUpTo(path_1.default.join(match1, ".."), "foo")).toEqual(root);
    });
});
...
- id: topic
  ...
  namespace: true
`, { encoding: "utf-8" });
        const schema = filesv2_1.file2Schema(fpath, root);
