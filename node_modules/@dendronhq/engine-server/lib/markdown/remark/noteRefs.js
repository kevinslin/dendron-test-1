...
exports.noteRefs = exports.convertNoteRefAST = void 0;
...
function convertNoteRefAST(opts) {
    let errors = [];
    const { link, proc, compilerOpts } = opts;
    const refLvl = utils_2.MDUtilsV4.getNoteRefLvl(proc());
    const { dest, vault } = utils_2.MDUtilsV4.getDendronData(proc);
...
exports.convertNoteRefAST = convertNoteRefAST;
function convertNoteRefHelperAST(opts) {
    ...
    let { anchorStart, anchorEnd, anchorStartOffset } = lodash_1.default.defaults(link.data, {
        anchorStartOffset: 0,
    });
    ...
    if (anchorStart) {
        if (lodash_1.default.trim(anchorStart).toLowerCase() === opts.title.toLowerCase()) {
