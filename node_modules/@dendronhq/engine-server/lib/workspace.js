...
class WorkspaceService {
    ...
    async createVault({ vault, noAddToConfig, }) {
        ...
        const note = common_all_1.NoteUtilsV2.createRoot({
            vault,
            body: [
                "# Welcome to Dendron",
                "",
        ...
        const schema = common_all_1.SchemaUtilsV2.createRootModule({ vault });
        if (!fs_extra_1.default.existsSync(common_all_1.NoteUtilsV2.getPathV4({ note, wsRoot: this.wsRoot }))) {
            await common_server_1.note2File({ note, vault, wsRoot: this.wsRoot });
        }
        if (!fs_extra_1.default.existsSync(common_all_1.SchemaUtilsV2.getPath({ root: this.wsRoot, fname: "root" }))) {
    ...
    async createVaultV2({ vault }) {
        const vaultFullPath = common_server_1.vault2Path({ vault, wsRoot: this.wsRoot });
        ...
        const schema = common_all_1.SchemaUtilsV2.createRootModule({ vault });
        const notePath = common_all_1.NoteUtilsV2.getPathV4({ note, wsRoot });
    ...
    static async createFromConfig(opts) {
        ...
        const ws = new WorkspaceService({ wsRoot });
        await Promise.all(config.vaults.map(async (vault) => {
     ...
     */
    async cloneVaultV2(opts) {
        const { vault } = opts;
        if (!vault.remote || vault.remote.type !== "git") {
            throw new common_all_1.DendronError({ msg: "cloning non-git vault" });
        }
        let remotePath = vault.remote.url;
        const localPath = common_server_1.vault2Path({ vault, wsRoot: this.wsRoot });
        const git = common_server_1.simpleGit();
    ...
    async cloneVault(opts) {
        ...
        const repoPath = common_all_1.WorkspaceUtilsCommon.getPathForVault({ wsRoot, vault });
        const repoDir = common_all_1.WorkspaceUtilsCommon.getRepoDir(wsRoot);
