...
const IMPORT = {
    ROOT_WITH_MULT_FOLDERS: new utilsv2_1.TestPresetEntryV4(async ({ wsRoot, engine, vaults, extra }) => {
        const { pod } = extra;
        const importSrc = common_server_1.tmpDir().name;
        await fileUtils_1.FileTestUtils.createFiles(importSrc, [
            { path: "foo.jpg" },
        ...
        let [actualFiles, expectedFiles] = fileUtils_1.FileTestUtils.cmpFiles(vpath, [
            "assets",
            "project.p1.md",
            "project.p1.n1.md",
            "project.p1.n2.md",
    ...
    SPECIAL_CHARS: new utilsv2_1.TestPresetEntryV4(async ({ wsRoot, engine, vaults, extra }) => {
        const { pod } = extra;
        const importSrc = common_server_1.tmpDir().name;
        await fileUtils_1.FileTestUtils.createFiles(importSrc, [
            // spaces
    ...
    CONVERT_LINKS: new utilsv2_1.TestPresetEntryV4(async ({ wsRoot, engine, vaults, extra }) => {
        const { pod } = extra;
        const importSrc = common_server_1.tmpDir().name;
        ...
        return [
            {
                expected: true,
                actual: await utils_1.AssertUtils.assertInString({
                    body,
                    match: ["[[project.p1.n1]]"],
                    nomatch: [],
                }),
